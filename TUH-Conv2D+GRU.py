...
def createModel():
    ...
    conv2d_1=Conv2D(filters=18,kernel_size=(4,1),strides=(2,1),padding='same',activation='relu')(inputs)
    conv2d_2=Conv2D(filters=9,kernel_size=(2,1),strides=(2,1),padding='same',activation='relu')(conv2d_1)
    conv2d_3=Conv2D(filters=1,kernel_size=(6,1),strides=(1,1))(conv2d_2)
...
def computeOverallAcc(sample='test',sample_train=None,label_train=None,sample_test=None,label_test=None,model=None):
    if sample is 'test':
        ...
        y_ans = label_train
        output = 'Train'
    y_pred_subs = model.predict(x_data,batch_size=BATCH_SIZE) #y_pred_subs.shape=(N, 50, 2)
